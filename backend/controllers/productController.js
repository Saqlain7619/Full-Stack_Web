...
exports.getFeatured = async (req, res) => {
  const products = await prisma.product.findMany({ where: { featured: true, active: true }, take: 8, include: { category: { select: { name:true, slug:true } } } });
  res.json({ success: true, products });
};

exports.createProduct = async (req, res) => {
  const { name, description, price, comparePrice, categoryId, stock, featured } = req.body;
  const images = parseImages(req.body, req.files);

...
exports.getCategory = async (req, res) => {
  const category = await prisma.category.findFirst({ where: { OR: [{ id: req.params.id }, { slug: req.params.id }] } });
...
exports.updateCategory = async (req, res) => {
  ...
  if (name) { data.name = name; data.slug = name.toLowerCase().replace(/\s+/g,'-').replace(/[^\w-]/g,''); }
  if (req.file) data.image = req.file.path || `/uploads/${req.file.filename}`;

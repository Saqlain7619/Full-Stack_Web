  ...
  return (
    <Link to={`/products/${product.slug || product.id}`} className="group card hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
      <div className="relative overflow-hidden aspect-square bg-gray-50">
        ...
        {discount && <span className="absolute top-3 left-3 badge bg-red-500 text-white">-{discount}%</span>}
        {product.stock === 0 && <div className="absolute inset-0 bg-white/60 flex items-center justify-center"><span className="badge bg-gray-800 text-white text-sm px-3 py-1">Out of Stock</span></div>}

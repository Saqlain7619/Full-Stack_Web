      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center h-16 gap-4">
          ...
          <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-lg mx-4">
            <div className="relative w-full">
              <input
                ...
                className="w-full pl-4 pr-10 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-gray-50"
              />
          ...
          <nav className="hidden lg:flex items-center gap-6">
            ...
            <Link to="/about" className="text-sm font-medium text-gray-600 hover:text-primary-600 transition-colors">About Us</Link>
            <Link to="/contact" className="text-sm font-medium text-gray-600 hover:text-primary-600 transition-colors">Contact</Link>
          </nav>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <Link to="/cart" className="relative p-2 rounded-xl hover:bg-gray-100 transition-colors">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-primary-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center font-bold">
              ...
              <div className="relative">
                ...
                {userOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-lg border border-gray-100 py-2 z-50">
                    ...
                    {user?.role === 'ADMIN' && (
                      <Link to="/admin" className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors text-primary-600">
                        <LayoutDashboard size={15} /> Admin Panel
                      </Link>
                    )}
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-gray-50 transition-colors w-full text-left text-red-500"
                    >
          ...
          <form onSubmit={handleSearch}>
            <div className="relative">
              <input
                ...
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search products..."
                className="w-full pl-4 pr-10 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-gray-50"
              />
        ...
        {menuOpen && (
          <div className="lg:hidden border-t border-gray-100 py-4 space-y-1">
            ...
            <Link to="/about" className="block px-4 py-2.5 text-sm font-medium hover:bg-gray-50 rounded-xl">About Us</Link>
            <Link to="/contact" className="block px-4 py-2.5 text-sm font-medium hover:bg-gray-50 rounded-xl">Contact</Link>
            {!token && (
              <Link to="/login" className="block px-4 py-2.5 text-sm font-medium text-primary-600">Sign In</Link>
            )}
          </div>
        )}
